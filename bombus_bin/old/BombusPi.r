...
#	Calculate Pi in Each Gene across whole genome  ------------------------
...
if(is.null(nrow(blah))){print("nope")}else{		
	if(nrow(blah)=="0"){print("nope")}else{	
		print(i)
		#logic from VCFFunctions_Diploids.r
		w = (genes[i,5]-genes[i,4])
		snps = x[SNPs[1,] == genes[i,1]]
		ones=sapply(snps[blah[,1]],function(x) (length(x[st:en][grep("^1:", x[st:en])]))) #homo ref
		twos=sapply(snps[blah[,1]],function(x) (length(x[st:en][grep("^0:", x[st:en])]))) #homo alt
		miss=sapply(snps[blah[,1]],function(x) (length(x[st:en][grep("[.]/[.]:", x[st:en])]))) 
