...
gm.bter = read.csv("/media/data1/bombus/Snipre/snipre_run/bayesian_results_BmelToBimp.csv", header=T)
...
ortho = read.table(file="/data2/bombus/workingFiles/OrthoGammaBIMPvAPIS.txt",header=T)
...
#Inconsistent social environment???
#On GO:
#Evolution within social structures does not lead to correlated ev. rates and does not lead to large overlap in terms evolving
	#DC KK paper

#On Brood versus Rep
	#Similar brain GE lead to similar ev history (reproductive versus brood care)
	#Reproductive genes won out. 
	# Noted that the BGE is highly correlated between poliste
...
plot(test$BSnIPRE.gamma.bter, test$gamma) 
